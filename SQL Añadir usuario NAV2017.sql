...
FROM [User]
WHERE [Windows Security ID] = @StringSID
...
IF EXISTS(SELECT *
FROM [Access Control]
WHERE [User Security ID] = @UserGUID AND [Role ID] = 'SUPER' AND [Company Name] = '')
...
IF EXISTS(SELECT *
FROM [User Property]
WHERE [User Security ID] = @UserGUID)
