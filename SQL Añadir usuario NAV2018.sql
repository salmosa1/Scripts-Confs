...
SET @UserID = 'DYNAMIZATIC\smorales'  -- Windows Login you want to add
...
DECLARE @j AS int
DECLARE @Grp AS int
DECLARE @BlankGUID uniqueidentifier
SET @BlankGUID = '00000000-0000-0000-0000-000000000000'
...
ELSE BEGIN
  ...
  -- Create User record
  INSERT INTO [User] ([User Security ID], [User Name], [Full Name], [State], [Expiry Date], [Windows Security ID], [Change Password], [License Type], [Authentication Email], [Application ID], [Contact Email], [Exchange Identifier])
  VALUES(@UserGUID, @UserID, '', 0, '1/1/1753', @StringSID, 0, 0, '', @UserGUID, '', @UserGUID)  
...
ELSE BEGIN  
  ...
  VALUES(@UserGUID, 'SUPER', '', @BlankGUID, 0) 
