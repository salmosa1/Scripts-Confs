...
#>
function Sign-BcContainerAppWithSHA1 {
    Param (
...
try {
    ...
    try {        
        Invoke-ScriptInBcContainer -containerName $containerName -useSession:$false -ScriptBlock { Param($appFile, $CertificateThumbprint, $timeStampServer, $digestAlgorithm)
            ...
            do {
                try {
                    & "$signtoolexe" @("sign", "/sha1", "$CertificateThumbprint", "/fd", $digestAlgorithm, "/td", $digestAlgorithm, "/tr", "$timeStampServer", "$appFile") | Write-Host                    
                    break
                } catch {
